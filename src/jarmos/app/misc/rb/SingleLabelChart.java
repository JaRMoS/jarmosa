...
public class SingleLabelChart extends LineChart {
								...
								// for UB
	{
		...
		if (!progressLabel) {
			if ((pointInSeries >= 0) && (pointInSeries < points.length)) {
				...
				while (accuracyPlace >= decimalPlace * 10)
					decimalPlace *= 10;
				...
				// this is essentially identical to LineChart's
				// drawChartValuesText, but it rounds the data
				// points depending on the range of values present, and only
				// plots for one series and one
	...
	public static String roundToNthPlace(double toRound, double accPlace) {
		...
		if (toRound < 0)
			forDecimalFormat += "-";
		for (int i = 0; i < placesBeforeDecimal; i++)
			forDecimalFormat += "0";
		if (placesAfterDecimal > 0)
			forDecimalFormat += ".";
		for (int i = 0; i < placesAfterDecimal; i++)
			forDecimalFormat += "0";
