 ...
 * This Activity handles downloading the stored data, initializing the systems and performing
 * the RB solve.
 * 
 * Copyright (C) 2010 David J. Knezevic and Phuong Huynh, Author made changes to the code.
 * 
 * @author Daniel Wirtz
 * @date Aug 10, 2011
 *
...
public class RBActivity extends Activity {
	 ...
	 * RB_solve.
	 * 
	 * Changed the default value to 1 as simulations with zero N makes little sense.
	 * Up to now, an error was thrown when N=1 from the model and thus the seekBar could not be moved, never invoking
	 * the onProgressChanged event and hence leaving this value at zero.
	 */
...
//			}
			...
			Log.d(DEBUG_TAG,
					"Error loading model, modeldir: " + MainActivity.modelmng.getModelDir());
	...
	final Handler downloadHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			Log.d(DEBUG_TAG,
					...
					+ ", model dir: "
							+ MainActivity.modelmng.getModelDir());
	...
	private class ModelLoader extends Thread {
		...
		private AModelManager m = MainActivity.modelmng;
...
//			}
			...
			try {
				if (mRbModel != null) {
					...
					Log.d(DEBUG_TAG,
							"Finished reading offline data for RBModel.");
				}
			} catch (Exception e) {
				Log.e(DEBUG_TAG,
						"Exception occurred while reading offline data: "
								+ e.getMessage(),e);
				success = false;
			}
