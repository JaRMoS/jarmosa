...
public class OutputPlotterActivity extends Activity {
	...
	private int add_sweep_pts_toggle; // 0 for adding points, 1 for showing
										// values, 2 for nothing
	...
	// stores the interpolated y-value indicating where a new sweep point is to
	// be graphed
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < n_outputs; i++) {
			for (int time_step = 0; time_step < n_time_steps; time_step++) {
				RB_outputs_LB[i][time_step] = RB_outputs_all_k[i][time_step]
						- RB_output_error_bounds_all_k[i][time_step];
				RB_outputs_UB[i][time_step] = RB_outputs_all_k[i][time_step]
						+ RB_output_error_bounds_all_k[i][time_step];
			}
		}
	...
	protected XYMultipleSeriesDataset buildDataset(String[] titles,
		...
		for (int i = 0; i < num_series; i++) {
			...
			for (int k = 0; k < seriesLength; k++) {
				// determine which values not to plot based on given xMin & xMax
				// value
				// do not add values for hidden plots
				if ((xV[k] >= xMin) && (xV[k] <= xMax) && !plotRemoved[i / 3])
					series.add(xV[k], yV[k]);
				if (yV[k] < yMin && !plotRemoved[i / 3])
					yMin = yV[k];
				if (yV[k] > yMax && !plotRemoved[i / 3])
					yMax = yV[k];
			}
			dataset.addSeries(series);
		}

		// an additional series for displaying what sweep values are to be
		// computed with a vertical line
	...
	public GraphicalView execute(Context context) {
		...
		// return ChartFactory.getLineChartView(context, buildDataset(labels, t,
		// output_values), renderer);
	...
	public boolean onTouchEvent(final MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (add_sweep_pts_toggle == 0)
				...
				showPoint(X / mChartView.getWidth());
	...
	public void add_sweep_point(float xpos) {
		if (sweepIndex > -1) {
			...
			// xrange is the current range of the graph's x-values (determined
			// by xMin and xMax)
			...
			// now, adjusting for the current leftmost point, find the smallest
			// time step greater than the touch point
			int closestTimestep = 0;
			while (time_step_array[closestTimestep + leftmostStep] < xAxisValue) {
				closestTimestep++;
			}

			// take the average of the values of the first visible dataset to
			// get the y axis value for showing
			// progress label
			int firstVisible = 0;
			while (plotRemoved[firstVisible])
				firstVisible++;
			if (firstVisible < n_outputs) { // there is a visible output

				int ts = closestTimestep + leftmostStep;
				// before computing the value, set progress values to indicate
				// the computation is taking place
				progressXVal = xcurrent;
				progressYVal = RB_outputs_all_k[firstVisible][ts - 1]
						+ ((RB_outputs_all_k[firstVisible][ts] - RB_outputs_all_k[firstVisible][ts - 1]) / (time_step_array[ts] - time_step_array[ts - 1]))
	...
	public void showPoint(float xpos) {
		...
		// now, adjusting for the current leftmost point, find the smallest time
		// step greater than the touch point
		int closestTimestep = 0;
		while (time_step_array[closestTimestep + leftmostStep] < xAxisValue) {
			closestTimestep++;
		}
		...
		// determine
		// which values get labeled instead of all of them (in the default
		// XYChart)
	...
	protected Dialog onCreateDialog(int id) {
		...
		case AXES_DIALOG_ID:
			...
			// handle changing local values to user-submitted xMin and xMax
			// values on clicking the ok button
			Button okButton = (Button) dialog.findViewById(R.id.axes_okButton);
			okButton.setOnClickListener(new View.OnClickListener() {

				public void onClick(View view) {
					// determine if value in minimum input field is within
					// acceptable range
					...
					// determine if value in maximum input field is within
					// acceptable range
		...
		case PLOT_DIALOG_ID:
			...
			for (int i = 0; i < n_outputs; i++) {
				if (plotRemoved[i] == true)
					plotBooleans[i] = false;
				else
					plotBooleans[i] = true;
			}
		...
		case LABEL_DIALOG_ID:
			...
			if (sweepIndex > -1) {
				...
				builder2.setMultiChoiceItems(labelStrings, labelBooleans,
						new DialogInterface.OnMultiChoiceClickListener() {
							public void onClick(DialogInterface dialog,
								...
								if (item == 1) {
									add_sweep_pts_toggle = 1; // now on touch,
																// data values
																// will show

									// determine whether more than one output is
									// showing: if this is the case,
									// recommend that the user disable all but
									// one output
									int visibleCount = 0;
									for (int i = 0; i < n_outputs; i++) {
										if (plotRemoved[i] == false)
											visibleCount++;
									}
									if (visibleCount > 1) {
				...
				if (add_sweep_pts_toggle == 1)
					labelBooleans[0] = true;
				else
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		inflater.inflate(R.menu.rb_plotter_main_menu, menu);
		return true;
	}
	...
	// solves for new sweep values separately from UI thread, allowing UI to
	// indicate that a solve
	// is being performed
	private class SolveThread extends Thread {
		...
		public void run() {
			...
			// time_step_array[xleft+1]-time_step_array[xleft+1];
