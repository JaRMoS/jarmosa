...
//	}
		...
		for (int ifn = 0; ifn < field_num; ifn++) {
			...
			if (Math.abs(val_max - val_min) > 1e-8) {
				...
				for (int i = 0; i < sol[0].length; i++) {
					float tmpvar = (sol[ifn][i] - val_min)
							/ (val_max - val_min);
					if (tmpvar <= 0.125f) {
	...
	public void read_offline_data(AModelManager m) throws IOException {
		String[] tokens = null;
		{
			try {
				...
				Log.e(this.getClass().getName(), "Error loading geometry.dat: "
						+ e.getMessage()+". Visual components most likely will not work and crash the application.",e);
				return;
			}
		}
	...
	// a row of LTfunc is the rowwise flatten of the [3,3] transformation matrix
	// and the [3,1] translation vector
	public void nodal_transform(float[][] LTfunc) {
		...
		for (int i = 0; i < node_num; i++) {
			...
			node[i * 3 + 0] = LTfunc[node_reg[i]][0] * old_node[0]
					+ LTfunc[node_reg[i]][1] * old_node[1]
					+ LTfunc[node_reg[i]][2] * old_node[2]
					+ LTfunc[node_reg[i]][9];
			node[i * 3 + 1] = LTfunc[node_reg[i]][3] * old_node[0]
					+ LTfunc[node_reg[i]][4] * old_node[1]
					+ LTfunc[node_reg[i]][5] * old_node[2]
					+ LTfunc[node_reg[i]][10];
			node[i * 3 + 2] = LTfunc[node_reg[i]][6] * old_node[0]
					+ LTfunc[node_reg[i]][7] * old_node[1]
					+ LTfunc[node_reg[i]][8] * old_node[2]
	...
	public void set_field_data(float[] _val1, float[] _val2, boolean isDeformed) {
		if (isDeformed) {
			...
			if ((_val1.length / node_num == 1)
					&& (_val2.length / node_num == 1)) {
				...
				for (int i = 0; i < node_num; i++)
					for (int j = 0; j < _vframe_num; j++) {
						vnode[j * node_num * 3 + i * 3 + 0] = vnode[j
								* node_num * 3 + i * 3 + 0]
								+ _val1[j * node_num + i] / sval;
						vnode[j * node_num * 3 + i * 3 + 1] = vnode[j
								* node_num * 3 + i * 3 + 1]
								+ _val2[j * node_num + i] / sval;
						vnode[j * node_num * 3 + i * 3 + 2] = vnode[j
								* node_num * 3 + i * 3 + 2];
	...
	public void set_field_data(float[] _val1, float[] _val2, float[] _val3,
		...
		if ((_val1.length / node_num == 1) && (_val2.length / node_num == 1)
			...
			for (int i = 0; i < node_num; i++)
				for (int j = 0; j < _vframe_num; j++) {
					vnode[j * node_num * 3 + i * 3 + 0] = vnode[j * node_num
							* 3 + i * 3 + 0]
							+ _val1[j * node_num + i] / sval;
					vnode[j * node_num * 3 + i * 3 + 1] = vnode[j * node_num
							* 3 + i * 3 + 1]
							+ _val2[j * node_num + i] / sval;
					vnode[j * node_num * 3 + i * 3 + 2] = vnode[j * node_num
							* 3 + i * 3 + 2]
							+ _val3[j * node_num + i] / sval;
